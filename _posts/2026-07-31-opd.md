---
layout: post
title: "On-Policy Distillation：为什么不应该对全词表计算多步 Advantage？"
date: 2026-02-20 09:00:00 +0800
categories: [research, llm, on-policy distillation, OPD, full Vocabulary]
tags: [OPD]
description: 笔者最近在做 MOPD 研究时发现，在计算 Advantage（教师-学生预测概率的 log-ratio）时，仅对学生采样的单个 token 计算已经效果很好。Kimi K3 技术报告中也消融了 top-k 枚举，同样未见性能或效率提升。有感而发，本文从理论角度严格讨论：在 On-Policy Distillation 中，为什么不应该对全词表计算多步 Advantage。
---

# On-Policy Distillation：为什么不应该对全词表计算多步 Advantage？

## 引言

笔者最近在做 MOPD 研究时发现，在计算 Advantage（教师-学生预测概率的 log-ratio）时，仅对学生采样的单个 token 计算已经效果很好。Kimi K3 技术报告中也消融了 top-k 枚举，同样未见性能或效率提升。有感而发，本文从理论角度严格讨论：**在 On-Policy Distillation 中，为什么不应该对全词表计算多步 Advantage**。

---

## 背景：On-Policy Distillation 的目标

知识蒸馏的目标是让 student $q_\theta$ 逼近 teacher $p$。从信息论角度，最自然的目标是最小化反向 KL 散度：
...
$$r_t = \log \frac{p(y_t|y_{<t}, x)}{q_\theta(y_t|y_{<t}, x)}, \qquad R_t = \sum_{t'=t}^{T} r_{t'}$$

这是标准的 REINFORCE 结构，$R_t$ 为从第 $t$ 步起的累积奖励。
...
这一约束与"改变随机变量的身份"无关——全词表求和在数学上就是期望操作，本身完全合法。**真正的约束来自轨迹一致性**：多步 return 要求 token 与其后续轨迹之间存在真实的条件依赖关系，而这在全词表枚举中无法以可行的方式保证。

因此，若希望利用全词表信息降低方差，唯一正确且高效的做法是退回到**单步即时奖励**——亦即 token-level KL 蒸馏的路线。
