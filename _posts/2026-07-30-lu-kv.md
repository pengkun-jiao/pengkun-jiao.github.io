...
其中 <span>$b\_{\ell,h}$</span> 表示第 <span>$\ell$</span> 层第 <span>$h$</span> 个注意力头的保留预算。
...
把有限的内存预算 <span>$B\_{\text{total}}$</span> 分配给 <span>$L \times H$</span> 个注意力头，每个头分配 <span>$b\_{\ell,h}$</span> 个 token 的保留额度：
